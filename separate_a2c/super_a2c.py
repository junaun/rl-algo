...
# fix of cannot import PyTorchObs
...
class BaseModel(nn.Module):
    ...
    def obs_to_tensor(self, observation: Union[np.ndarray, Dict[str, np.ndarray]]) -> Tuple[PyTorchObs, bool]:
        ...
        if not isinstance(observation, dict):
            ...
            # Add batch dimension if needed
            # modified
            new_tuple = tuple(x+1 for x in self.observation_space.shape)
            observation = observation.reshape((-1, *new_tuple))  # type: ignore[misc]
...
class SuperActorCriticPolicy(BasePolicy):
    """
    ...
    def __init__(
        ...
        if isinstance(observation_space, spaces.Dict):
            observation_space = self._crop_observation(observation_space)
        else:
            observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(observation_space.shape[0] - 1,),
                                        dtype=np.float64)
            # observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(27,),
            #                             dtype=np.float64)
        ...
        # modified
        self.crop_start = 0 
        self.crop_end = self.observation_space.shape[0]
    ...
    # modified
    def _crop_observation(self, observation_space):
        if isinstance(observation_space, spaces.Dict):
            # for spaces.Dict or TensorDict
            new_obs_dict = {}
            for key, value in observation_space.spaces.items():
                if key in self.selected_keys:
                    new_obs_dict[key] = value
            return spaces.Dict(new_obs_dict)
            ...
            observation_space = observation_space[..., self.crop_start:self.crop_end]
...
class ActorCriticCnnPolicy(SuperActorCriticPolicy):
    """
...
class MultiInputActorCriticPolicy(SuperActorCriticPolicy):
    """
